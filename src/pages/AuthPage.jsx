...
export default function AuthPage({ setPage }) {
  ...
  if (step === 'confirm') return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh',padding:'2rem'}}>
      <div style={{background:'rgba(5,4,16,0.95)',border:'1px solid rgba(201,168,76,0.3)',borderRadius:16,width:'100%',maxWidth:420,padding:'2.5rem',textAlign:'center',boxShadow:'0 12px 48px rgba(0,0,0,0.8)',animation:'fadeUp .4s ease'}}>
        <div style={{fontSize:'2.5rem',marginBottom:'1rem'}}>✦</div>
        <h2 style={{fontFamily:"'Cinzel Decorative',serif",fontSize:'1.3rem',color:'#c9a84c',margin:'0 0 .8rem',letterSpacing:'.1em'}}>Check your email</h2>
        <p style={{color:'#c0b8d8',fontFamily:"'Crimson Pro',serif",fontSize:'1.05rem',margin:'0 0 .5rem',lineHeight:1.6}}>
          We sent a confirmation link to:
        </p>
        <p style={{color:'#c9a84c',fontFamily:"'JetBrains Mono',monospace",fontSize:'.85rem',margin:'0 0 1.5rem',letterSpacing:'.08em'}}>{success}</p>
        <p style={{color:'#7060a0',fontFamily:"'Crimson Pro',serif",fontSize:'.95rem',margin:'0 0 2rem',lineHeight:1.6}}>
          Click the link in the email to activate your account, then come back and sign in.
        </p>
        <div style={{background:'rgba(201,168,76,0.08)',border:'1px solid rgba(201,168,76,0.2)',borderRadius:10,padding:'12px 16px',marginBottom:'1.5rem'}}>
          <p style={{color:'#8878a0',fontFamily:"'JetBrains Mono',monospace",fontSize:'.68rem',letterSpacing:'.12em',margin:0}}>⚠ CHECK YOUR SPAM FOLDER IF YOU DON'T SEE IT</p>
        </div>
        <button onClick={()=>{setStep(1);setMode('signin');setEmail(success)}} style={{background:'rgba(201,168,76,0.12)',border:'1px solid rgba(201,168,76,0.4)',color:'#c9a84c',borderRadius:10,padding:'10px 28px',fontFamily:"'JetBrains Mono',monospace",fontSize:'.78rem',letterSpacing:'.12em',cursor:'pointer'}}>
          Go to sign in →
        </button>
      </div>
    </div>
